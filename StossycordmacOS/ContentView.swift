...
struct SidebarView: View {
    ...
    @State private var isDMaActive = false  // State to control the default navigation
    let keychain = KeychainSwift()
    
    var body: some View {
        NavigationView {
            List {
                NavigationLink(destination: DMa(webSocketClient: webSocketClient, token: token, username: username),
                               isActive: $isDMaActive) {
                    Label("DMs", systemImage: "star")
                        .onAppear() {
                ...
                .onAppear {
                    // Set isDMaActive to true when the view appears to make DMa the default view
                    if !isDMaActive {
                        DispatchQueue.main.async {
                            self.isDMaActive = true
                        }
                    }
                }
...
func uploadFileToDiscord2(fileUrl: URL, token: String, channelid: String, message: String, messageReference: [String: String]? = nil) {
    ...
    // Add messageReference to the request body if provided
    if let messageReference = messageReference {
        var bodyObject: [String: Any] = ["content": message]
        bodyObject["message_reference"] = messageReference
        request.httpBody = try? JSONSerialization.data(withJSONObject: bodyObject)
    }
